...
st.sidebar.write(f"✔️ Válidas: {len(linhas_validas)}")
st.sidebar.write(f"❌ Inválidas: {len(linhas_invalidas)}")
if linhas_invalidas:
...
st.sidebar.caption("🧭 Colunas encontradas:")
...
with aba4:
    ...
    if 'Valor' in df_filtrado.columns:
        ...
        if 'Fornecedor' in df_filtrado.columns:
            ...
            fig_gf = px.bar(gasto_forn.sort_values(by='Valor', ascending=False),
